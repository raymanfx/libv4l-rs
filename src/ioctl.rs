...
macro_rules! _IOC {
    ($dir:expr, $type:expr, $nr:expr, $size:expr) => {
        (($dir as _IOC_TYPE) << $crate::ioctl::_IOC_DIRSHIFT)
            | (($type as _IOC_TYPE) << $crate::ioctl::_IOC_TYPESHIFT)
            | (($nr as _IOC_TYPE) << $crate::ioctl::_IOC_NRSHIFT)
            | (($size as _IOC_TYPE) << $crate::ioctl::_IOC_SIZESHIFT)
    };
}

macro_rules! _IO {
    ($type:expr, $nr:expr) => {
        _IOC!($crate::ioctl::_IOC_NONE, $type, $nr, 0)
    };
}

macro_rules! _IOR {
    ($type:expr, $nr:expr, $size:ty) => {
        _IOC!($crate::ioctl::_IOC_READ, $type, $nr, _IOC_TYPECHECK!($size))
    };
}
