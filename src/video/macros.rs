...
macro_rules! impl_format {
    ($typ:expr, $pix_key:ident, $fmt_type:ident) => {
        fn format(&self) -> io::Result<$fmt_type> {
            unsafe {
                ...
                Ok($fmt_type::from(v4l2_fmt.fmt.$pix_key))
            }
        }
    };
}
...
macro_rules! impl_set_format {
    ($typ:expr, $pix_key:ident, $fmt_type:ident, $device:ident) => {
        fn set_format(&self, fmt: &$fmt_type) -> io::Result<$fmt_type> {
            unsafe {
                let mut v4l2_fmt = v4l2_format {
                    type_: $typ as u32,
                    fmt: v4l2_format__bindgen_ty_1 { $pix_key: (*fmt).into() },
                };
                v4l2::ioctl(
                    self.handle().fd(),
